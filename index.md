## Privacy Policy for Picto AI Photo Editor

**Last updated:** January 30, 2026

Picto AI (“we”, “our”, or “us”) respects your privacy and is committed to protecting it. This Privacy Policy explains how your information is collected, used, and handled when you use the GlowUp AI mobile application.

---

### 1. Information We Collect

Picto AI may collect the following types of information:

#### a. Photos and Videos
- Photos or videos are collected **only when you choose to upload them**
- These are used **solely to provide app functionality** (AI image processing)
- Media is **not used for advertising or tracking**

#### b. User Account Information
- Email address or user ID (used for authentication and account management)
- This information is **not shared with third parties for advertising**

#### c. Purchase Information
- In-app purchase data is used to manage credits and transactions
- Purchase history is **not used for tracking or marketing**

---

### 2. How We Use Your Information

Your data is used only for:
- App functionality
- Account authentication
- Credit and purchase management
- Improving app stability and performance

We **do not**:
- Sell your data
- Share personal data with advertisers
- Track users across other apps or websites

---

### 3. Data Storage and Security

- Data is transmitted securely using industry-standard encryption
- Images may be temporarily processed on secure servers
- We take reasonable measures to protect your information

---

### 4. Third-Party Services

Picto AI uses trusted third-party services for:
- Authentication
- Payments (Apple In-App Purchases)
- Backend infrastructure

These services are only provided access to information necessary to perform their functions.

---

### 5. Children’s Privacy

Picto AI is **not intended for children under the age of 13**. We do not knowingly collect personal data from children.

---

### 6. Your Rights

You may:
- Request deletion of your account and associated data
- Contact us regarding privacy concerns

---

### 7. Changes to This Policy

We may update this Privacy Policy from time to time. Changes will be reflected on this page.

---

### 8. Contact Us

If you have questions about this Privacy Policy, contact us at:

📧 **Email:** rithikmotupalli@gmail.com
